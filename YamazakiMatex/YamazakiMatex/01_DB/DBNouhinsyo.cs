...
namespace DB
{
    ...
    /// 納品書DBモジュール
    /// </summary>
    class DBNouhinsyo : DBFileLayout
    {
        #region 変数宣言
        /// <summary>
        /// 受注データ格納変数
        ...
        public class JuchuDataClass
        {
            ...
            public void setHeaderData(DataTable headerData)
            {
                ...
                // 伝票日付
                HeaderData.DenpyoHizuke = CommonLogic.ToDateTime(headerRow[JuchuHeaderFile.dcDenpyoHizuke]);
                // 受注NO(担当者コード)
                ...
                // 着日
                HeaderData.Tyakubi = CommonLogic.ToDateTime(headerRow[JuchuHeaderFile.dcTyakubi]);
                // 出荷便
            ...
            public void setBodyData(DataTable bodyData)
            {
                ...
                for (int i = 0; i < bodyData.Rows.Count; i++)
                {
                    ...
                    // 納品数量合計
                    if (bodyData.Columns.Contains(JuchuBodyFile.dcTotalNouhinSuryo))
                    {
                        wkJuchuBodyInfo.TotalNouhinSuryo = CommonLogic.ToDecimal(bodyRow[JuchuBodyFile.dcTotalNouhinSuryo]);
                    }
                    // 仕入数量合計
                    if (bodyData.Columns.Contains(JuchuBodyFile.dcTotalShireSuryo))
                    {
        ...
        public class UriageDataClass
        {
            ...
            public void setHeaderData(DataTable headerData)
            {
                ...
                // 客先注番
                HeaderData.KyakusakiChuban = Convert.ToString(headerRow[UriageHeaderFile.dcKyakusakiChuban]);
                // 請求日付
        ...
        public class ShireDataClass
        {
            ...
            public void setBodyData(DataTable bodyData)
            {
                ...
                for (int i = 0; i < bodyData.Rows.Count; i++)
                {
                    ...
                    // 締年月日
                    if (string.IsNullOrEmpty(Convert.ToString(bodyRow[ShireBodyFile.dcShimeNengapi])))
                    {
                        wkShireBodyInfo.ShimeNengapi = null;
                    }
                    else
                    {
                        wkShireBodyInfo.ShimeNengapi = Convert.ToDateTime(Convert.ToString(bodyRow[ShireBodyFile.dcShimeNengapi]));
                    }
                    // 締更新フラグ
        ...
        /// <returns></returns>
        public SelectErrorType getJuchuData(string juchuDocumentNo, bool flgInit)
        {
            if (flgInit)
            {
        ...
        /// <returns></returns>
        public SelectErrorType getNouhinsyoData(string nouhinDocumentNo)
        {
            ...
            if (uriageHeaderData != null && uriageHeaderData.Rows.Count > 0)
            {
                ...
                // 売上ボディデータ取得処理実行
                uriageBodyData = executeSelect(sqlUriageBodyCommand, false);
                // 取得時にエラーが発生した場合
                if (DBRef < 0) return SelectErrorType.UriageBody;
                // 売上フッタデータ取得処理実行
                uriageFooterData = executeSelect(sqlUriageFooterCommand, false);
                // 取得時にエラーが発生した場合
            ...
            // 対象売上データに紐付く受注データを取得
            res = getJuchuData(getJuchuDocumentNo(SelectUriageData.HeaderData.JuchuNoTop
                                                , SelectUriageData.HeaderData.JuchuNoMid
                                                , SelectUriageData.HeaderData.JuchuNoBtm)
                                                , false);
            // 取得時にエラーが発生した場合
            if (res != SelectErrorType.None) return res;

            // 対象売上データに紐付く仕入データを取得
            res = getShireData(string.Empty, SelectUriageData.HeaderData.DenpyoNo, false);
        ...
        private string getJuchuDocumentNo(string juchuNoTop, string juchuNoMid, string juchuNoBtm)
        {
            ...
            DataTable dt = executeSelect(sql, false);
            if (dt != null && dt.Rows.Count > 0)
            {
                result = Convert.ToString(dt.Rows[0][JuchuHeaderFile.dcDenpyoNo]);
                sql = "SELECT * FROM juchu_header ";
                sql += "WHERE denpyoNo = '" + result + "' ";
                dt = executeSelect(sql, true);
            }
        ...
        /// <param name="flgInit"></param>
        /// <returns></returns>
        private SelectErrorType getShireData(string shireNo, string nouhinDocumentNo, bool flgInit)
        {
            if (flgInit)
            {
                SelectJuchuData = new JuchuDataClass();
            ...
            // 仕入Noが空白の場合、売上Noから仕入Noを取得
            if (string.IsNullOrEmpty(shireNo))
            {
                string sql = "SELECT * FROM shire_header WHERE uriageDenpyoNo = '" + nouhinDocumentNo + "' AND (kanriKubun IS NOT NULL AND kanriKubun <> '9') ";
                DataTable dt = executeSelect(sql, false);
                if (dt != null && dt.Rows.Count > 0) shireNo = Convert.ToString(dt.Rows[0][ShireHeaderFile.dcShireNo]);
            }
            // 仕入ヘッダデータ取得SQL生成
            sqlHeaderCommand = "SELECT * FROM shire_header WHERE shireNo = '" + shireNo + "' AND (kanriKubun IS NOT NULL AND kanriKubun <> '9') ";
            ...
            if (shireHeaderData != null && shireHeaderData.Rows.Count > 0)
            {
                ...
                // 仕入ボディデータ取得処理実行
                shireBodyData = executeSelect(sqlBodyCommand, true);
                // 取得時にエラーが発生した場合
                if (DBRef < 0) return SelectErrorType.ShireBody;
                // 仕入フッタデータ取得処理実行
                shireFooterData = executeSelect(sqlFooterCommand, true);
                // 取得時にエラーが発生した場合
