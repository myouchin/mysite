...
namespace SubForm
{
    ...
    /// 仕入検索画面
    /// </summary>
    public partial class sfrmShireSearch : ChildBaseForm
    {
        #region 変数宣言
        ...
        public class SelectedShireInfo
        {
            #region 変数宣言
            /// <summary>
            /// 仕入No
        ...
        private List<SelectedShireInfo> selectedShireInfos;
        public List<SelectedShireInfo> SelectedShireInfos
        {
            get { return selectedShireInfos; }
            set { selectedShireInfos = value; }
        }
        ...
        public sfrmShireSearch(bool flgMultiSelect, CheckMessageType checkMessageType)
        {
            InitializeComponent();
            commonLogic = new CommonLogic();
            SelectedShireInfos = new List<SelectedShireInfo>();
        ...
        /// <param name="e"></param>
        private void sfrmShireSearch_Load(object sender, EventArgs e)
        {
            setGridData();
        }
        #endregion
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            SelectedShireInfo wkSelectedShireInfo;
            // 選択行情報を格納
            foreach (DataGridViewRow gRow in grdSearchList.SelectedRows)
            {
                wkSelectedShireInfo = new SelectedShireInfo();
                wkSelectedShireInfo.ShireNo = Convert.ToString(gRow.Cells[clmPurchaseNo.DisplayIndex].Value);
                wkSelectedShireInfo.OrdersNo = Convert.ToString(gRow.Cells[clmOrdersNo.DisplayIndex].Value);
                SelectedShireInfos.Add(wkSelectedShireInfo);
            }
        ...
        private void setGridData()
        {
            ...
            selectSql += ", kenmei1 ";
            selectSql += "FROM shire_header ";
            selectSql += "WHERE 1 = 1 ";
            // 伝票No(FROM)が入力されている場合
            if (!string.IsNullOrEmpty(txtDocumentNoFrom.Text))
            {
                selectSql += "AND shireNo >= '" + txtDocumentNoFrom.Text + "' ";
            }
            // 伝票No(TO)が入力されている場合
            if (!string.IsNullOrEmpty(txtDocumentNoTo.Text))
            {
                selectSql += "AND shireNo <= '" + txtDocumentNoTo.Text + "' ";
            }
            ...
            if (rdoAsc.Checked)
            {
                selectSql += "ORDER BY denpyoHizuke, shireNo ";
            }
            else
            {
                selectSql += "ORDER BY denpyoHizuke DESC, shireNo DESC ";
            }
            // データ取得を実行
