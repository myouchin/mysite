...
namespace PrintInstructions
{
    ...
    public partial class frmSeikyusyo : ChildBaseForm
    {
        ...
        public frmSeikyusyo()
        {
            ...
            rdoAll.Location = new Point(7, 35);
            rdoPrint1.Location = new Point(80, 35);
            rdoPrint2.Location = new Point(167, 35);
        ...
        private dtblSeikyusyo getData()
        {
            ...
            if (Consts.EndOfMonthText.Equals(cmbTightenType.Text))
            {
                targetDateFrom = Convert.ToDateTime(targetDateTo.Year + "/" + targetDateTo.Month + "/01");
            }
            ...
            sql += "AND (tm.seikyuSyuturyokuKubun = '2' OR tm.seikyuSyuturyokuKubun = '3' OR tm.seikyuSyuturyokuKubun = '4') " + "\r";
            //sql += "ORDER BY ts.tokuisakiCode, ts.jigyousyoCode, ts.seikyubi " + "\r";
            sql += "ORDER BY ts.seikyuNo " + "\r";
            ...
            sql += "ORDER BY sh.denpyoHizuke, sh.denpyoNo, sb.rowNo ";
            ...
            foreach (DataRow hdRow in headDt.Rows)
            {
                ...
                togetuZeikomi += dec;
                if (flgKurikoshiSyuturyoku == decimal.Zero)
                {
                    headRow["togetuZeikomi"] = togetuZeikomi.ToString("#,##0");
                }
                else if (decimal.TryParse(Convert.ToString(hdRow[DBFileLayout.TokuisakiSeikyuFile.dcZeikomiSeikyu]), out togetuZeikomi))
                {
                    headRow["togetuZeikomi"] = togetuZeikomi.ToString("#,##0");
                }
                    ...
                    foreach (var query in groupQuery)
                    {
                        ...
                        foreach (DataRow bdRow in query)
                        {
                            ...
                            if (!flgMeisaiIkkatuSyuturyoku || (flgMeisaiIkkatuSyuturyoku && denpyoRowNo == 1))
                            {
                                ...
                                {
                                    // 伝票タイトル行追加
                                    kyakusakiChuban = Convert.ToString(bdRow[DBFileLayout.JuchuHeaderFile.dcKyakusakiChuban]);
                                    denpyoRowNo++;
                                    rowNo++;
                                    bodyRow = seikyusyo.Tables["dtblSeikyusyoB"].NewRow();
                                    bodyRow["grpNo"] = groupNo;
                                    ...
                                    bodyRow["hinmei1"] = Convert.ToString(bdRow["kenmei1"]);
            ...
            {
                parentShoukei = decimal.Zero;
                parentShouhizei = decimal.Zero;
            ...
            if (rdoAll.Checked  || rdoPrint2.Checked)
            {
                ...
                if (rdoPrint2.Checked)
                {
                    seikyusyo.Tables["dtblSeikyusyoH"].Rows.Clear();
                    seikyusyo.Tables["dtblSeikyusyoH"].AcceptChanges();
                    seikyusyo.Tables["dtblSeikyusyoB"].Rows.Clear();
                    seikyusyo.Tables["dtblSeikyusyoB"].AcceptChanges();
                }
                int newGroupNo = 0;
                int newPage = 0;
