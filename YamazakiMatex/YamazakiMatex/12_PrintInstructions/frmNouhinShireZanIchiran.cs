...
namespace PrintInstructions
{
    ...
    public partial class frmNouhinShireZanIchiran : ChildBaseForm
    {
        ...
        private dtblNouhinShireZanIchiran createPrintData()
        {
            ...
            sql += "     , sh.tokuisakiName ";
            sql += "     , sh.jigyousyoName ";
            ...
            sql += "ORDER BY sh.tokuisakiCode, sh.shiresakiCode, sb.shouhinCode ";
            ...
            #region 帳票ボディデータ設定
            foreach (DataRow juchuRow in juchuDt.Rows)
            {
                ...
                {
                    shiresakiName = Convert.ToString(shireRow[DBFileLayout.ShireHeaderFile.dcShiresakiName]);
                ...
                newRow["shiresakiName"] = shiresakiName;
            ...
            // 仕入明細の取得
            var shireGroupRows = shireDt.AsEnumerable().Where(p => p.Field<string>(DBFileLayout.ShireHeaderFile.dcJuchuNoTop).Equals(string.Empty) &&
                                                                   p.Field<string>(DBFileLayout.ShireHeaderFile.dcJuchuNoMid).Equals(string.Empty) &&
                                                                   p.Field<string>(DBFileLayout.ShireHeaderFile.dcJuchuNoBtm).Equals(string.Empty) &&
                                                                   p.Field<Int32>(DBFileLayout.ShireBodyFile.dcJuchuRowNo) == 0)
                                                       .GroupBy(p => new {
                                                           key1 = p.Field<string>(DBFileLayout.ShireHeaderFile.dcTokuisakiCode)
                                                                         ,
                                                           key2 = p.Field<string>(DBFileLayout.ShireHeaderFile.dcShiresakiCode)
                                                                         ,
                                                           key3 = p.Field<string>(DBFileLayout.ShireBodyFile.dcShouhinCode)
                                                       });
            foreach (var rows in shireGroupRows)
            {
                count = 0;
                nyukaSuryo = decimal.Zero;
                nyukaKingaku = decimal.Zero;
                ...
                {
                    count++;
                    tokuisakiName = Convert.ToString(row[DBFileLayout.ShireHeaderFile.dcTokuisakiName]);
                    jigyousyoName = Convert.ToString(row[DBFileLayout.ShireHeaderFile.dcJigyousyoName]);
                    shiresakiName = Convert.ToString(row[DBFileLayout.ShireHeaderFile.dcShiresakiName]);
                    shouhinName1 = Convert.ToString(row[DBFileLayout.ShireBodyFile.dcShouhinName1]);
                    shouhinName2 = Convert.ToString(row[DBFileLayout.ShireBodyFile.dcShouhinName2]);
                    decimal.TryParse(Convert.ToString(row[DBFileLayout.ShireBodyFile.dcSuryo]), out wkDec);
                    nyukaSuryo += wkDec;
                    decimal.TryParse(Convert.ToString(row[DBFileLayout.ShireBodyFile.dcKingaku]), out wkDec);
                    nyukaKingaku += wkDec;
                    if (count == rows.Count())
                    {
                        newRow = res.Tables["dtblNouhinShireZanIchiran"].NewRow();
                        newRow["juchuBi"] = string.Empty;
                        newRow["juchuNo"] = string.Empty;
                        newRow["tokuisakiName"] = tokuisakiName;
                        newRow["jigyousyoName"] = jigyousyoName;
                        newRow["juchuSuryo"] = string.Empty;
                        newRow["nouhinZanSuryo"] = string.Empty;
                        newRow["juchuKingaku"] = string.Empty;
                        newRow["nouhinZanKingaku"] = string.Empty;
                        newRow["shiresakiName"] = shiresakiName;
                        newRow["shouhinName1"] = shouhinName1;
                        newRow["shouhinName2"] = shouhinName2;
                        newRow["shireSuryo"] = nyukaSuryo.ToString("#,##0");
                        newRow["shireZanSuryo"] = decimal.Zero.ToString("#,##0");
                        newRow["shireKingaku"] = nyukaKingaku.ToString("#,##0");
                        newRow["shireZanKingaku"] = decimal.Zero.ToString("#,##0");
                        res.Tables["dtblNouhinShireZanIchiran"].Rows.Add(newRow.ItemArray);
                    }
                }
            }
